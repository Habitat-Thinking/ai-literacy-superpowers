...
**Cost folds into those gates, it does not add one.** Two of these gates also surface a prospective cost estimate — at Slice Adjudication (after carpaccio, a per-slice breakdown so you can see cost while choosing which slice to progress) and at Plan Approval (a tighter, spec-grounded estimate). Both arrive as *informational fields*, under the same rule as `cartograph_pending_count`: no block, no extra keypress, no agent writing a disposition, and no verdict — the read-only `cost-estimator` emits, the orchestrator writes, and you read the ranges while making the *existing* choice. If an estimate is unavailable, the gate proceeds exactly as before. Earlier still, *before* carpaccio runs, a coarse low-confidence **T0 ballpark** from the raw task text gives a quick go/no-go sniff-test — deliberately ephemeral (surfaced once, never persisted) so the least-accurate number never reads as fact. See [Prospective Cost Estimation](prospective-cost-estimation.md) for the fold-in mechanism and the T0 ballpark.
