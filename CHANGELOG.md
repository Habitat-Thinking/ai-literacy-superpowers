...
constriction"* at a glance is a picture: the loop drawn as a circle, with live
queue depth at each of its eight stages.

```bash
/harness-board --html            # temp file, opened, path printed
/harness-board --html out.html   # written where asked
```

Eight stages — assay, propose, review, dispose, cost at the gate, in force,
check, expiry — each carrying what is queued there when the page is generated.
The cycle is real rather than illustrative: a rule that expires returns to the
assay that would observe it again.

### The parent spec ruled this out, and the reason is why it can be revisited

`2026-08-25-harness-board-design.md` listed an HTML view under **Out of scope**,
because *"a file that must be regenerated becomes a file that is stale"* — with
two dashboards in this repository, 106 days unread, as the evidence.

That argues against a **committed artefact**, not against HTML. A page written to
a temporary location on every invocation has neither half of the failure mode:
it does not persist to be found later, and it is regenerated by definition.

### The constraint is now a refusal rather than a note
...
A path inside the working tree is rejected unless git already ignores it —
verified adversarially, including that **no file is written** on refusal, and
that a gitignored path is allowed so the tree stays clean either way.

That is the point of the design rather than a guard rail. "Do not commit this" as
a note is a note; a tool that cannot produce a committable file cannot become the
thing nobody regenerates. The note failed for two dashboards already.

### Other properties

- The page carries its own generation timestamp and states that it is a snapshot,
  so a reader who finds it later knows what they are looking at
- Self-contained: no external stylesheet, script or image beyond Google Fonts
- Failing to open a browser is **not an error** — the printed path is the result,
  which matters in a headless session or a container
- `git status` unchanged after any successful run
