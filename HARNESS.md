...
**Four constraints failing.** (1) *Release traceability* — `v0.28.1`–`v0.28.5` and `v0.29.1` carry CHANGELOG headings and no git tag, absent locally and on the remote, while `v0.28.0` and `v0.29.0` exist; the gap is patch releases in the primary plugin and it survived both the 2026-06-13 governance audit that scored this rule down and the 2026-08-13 audit. (2) *Output validation checkpoints* — `/harness-audit` writes this block, which has at least three downstream parsers, and `commands/harness-audit.md` has no read-back step; it is also absent from CLAUDE.md's checkpoint list, so this is an omission never noticed rather than a regression. (3) *PRs have adjudicated objections* — 6 of 6 non-exempt PRs (#552, #560, #561, #562, #566, #568) carry no objection record in either mode: twelve missing. The one record in the window, `docs/superpowers/objections/harness-provenance-citation.md`, is 12/12 `disposition: pending` with null rationales, and two merged specs cite it as their evidence base. (4) *PRs have adjudicated choice stories* — the same 6; `docs/superpowers/stories/` has received no record since the Cadence Sentinels epic.
