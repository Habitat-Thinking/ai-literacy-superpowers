...
**Stale claims in this file.** `### Stack` says "Test framework: None" while three active constraints and `tdad-tests-fast.yml` enforce a 150-test pytest suite, and all three generated mirrors carry the stale line. The *Consistent markdown formatting* rule claims local and CI check the same set; they do not — a bare local run is 511 files/91 issues against CI's 502/0, because `cost-estimates/**` and `diagnostic-legibility/output/**` are gitignored but absent from `.markdownlint-cli2.jsonc`. Committed markdown is genuinely clean; the equivalence claim is not. The template-version marker reads 0.79.0 against installed plugin 0.87.0 — eight minors of unreviewed template content. (This sentence first read "0.66.2 against 0.86.0": the marker figure was copied from the previous Status block without checking, in the block being rewritten for staleness. Corrected 2026-08-25 by the first on-demand /harness-gc run.)
