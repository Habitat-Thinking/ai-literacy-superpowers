...
**Three constraints failing**, and one repaired since this block was written. (1) *Release traceability* — `v0.28.1`–`v0.28.5` and `v0.29.1` carry CHANGELOG headings and no git tag, absent locally and on the remote, while `v0.28.0` and `v0.29.0` exist; the gap is patch releases in the primary plugin and it survived both the 2026-06-13 governance audit that scored this rule down and the 2026-08-13 audit. (2) *Output validation checkpoints* — **repaired in #577**: `/harness-audit` step 5 is now a mandatory validation checkpoint and the command is listed in CLAUDE.md. It was failing when this block was written, because the command that writes this block had no read-back step. (3) *PRs have adjudicated objections* — 6 of 6 non-exempt PRs (#552, #560, #561, #562, #566, #568) carry no objection record in either mode: twelve missing. The one record in the window, `docs/superpowers/objections/harness-provenance-citation.md`, is 12/12 `disposition: pending` with null rationales, and two merged specs cite it as their evidence base. (4) *PRs have adjudicated choice stories* — the same 6; `docs/superpowers/stories/` has received no record since the Cadence Sentinels epic.

**One masking defect with a measured cost.** GC run 32712709028 (2026-08-24) failed at step 5 (*Snapshot staleness*) and recorded steps 6–14 `skipped`, including step 9 *Release tag completeness* and step 13 *Reflection log archival (Path 1)* — the two auto-fix rules that would have closed failures (1) and the archival backlog. Six consecutive scheduled runs have failed the same way since 2026-07-20. Only `Summary` carries `if: always()`. `.github/workflows/harness.yml` has the same shape, and on runs 32836457544 (#552) and 32839441437 (#561) a ShellCheck failure left *Harness decision records are well-formed* and *Harness governance is applied and undrifted* `skipped` — on two PRs that were rewriting the governance validators.

**Enforcement-surface drift, both directions.** *Affordances have matching permissions* and *Permissions have declared affordances* both declare `Scope: pr` and name `harness-affordance-check.sh`; no workflow invokes it (the script passes both directions when run by hand). Inversely, GC rule *Affordance review staleness* runs as an active step at `gc.yml:258` while its declaring block sits inside an HTML comment and parses as inactive. And `dynamic-workflows-firewall.yml` runs `inv-firewall.sh` on every PR under no declared constraint at all.

**Stale claims in this file.** `### Stack` says "Test framework: None" while three active constraints and `tdad-tests-fast.yml` enforce a 150-test pytest suite, and all three generated mirrors carry the stale line. The *Consistent markdown formatting* rule claims local and CI check the same set; they do not — a bare local run is 511 files/91 issues against CI's 502/0, because `cost-estimates/**` and `diagnostic-legibility/output/**` are gitignored but absent from `.markdownlint-cli2.jsonc`. Committed markdown is genuinely clean; the equivalence claim is not. The unreviewed-template-backlog claim that stood here is gone with the mechanism that produced it: the version marker, its weekly GC rule and the SessionStart nudge were all retired in v0.90.0, which is why the GC count moved 19 → 18. That claim was itself the evidence for the retirement — it asserted eight minors of unreviewed content against a template that had not changed in twelve days, having been copied forward from a previous block without being checked.

**Reflection log + archival coverage.** 56 active fragments, 0 archived. 17 carry a `**Promoted**:` marker; `archive-promoted-reflections.sh --dry-run=true` reports 15 archivable and 2 failing `verify_rhs` (unchanged from 2026-08-13, reported rather than silently edited). Curation debt is 0 — the oldest active fragment is 2026-04-06, 141 days, inside the 180-day threshold. Path 1 (*Reflection log archival of promoted entries*, deterministic, auto-fix true) and Path 2 (*Reflection log aged-out review*, agent, monthly) are both declared. Path 1 is declared and not operating: it is step 13 of the GC job and has been masked for six weeks. Note that `archive-promoted-reflections.sh` defaults to `--dry-run=false`, so it writes unless told otherwise.

**Verified passing, observed rather than assumed:** gitleaks (611 commits, no leaks), `bash -n` and strict-mode sweeps, ShellCheck 0.11.0 (zero output), `mkdocs build --strict` (exit 0), TDAD 150 tests across 7 modules, convention parity 36/36 across all three mirrors plus 3 enums/45 member checks, sentinel integrity 10 agents, `check-harness-decisions.py` 3 HDRs, `harness-registrar.py check` 3 decisions, objection taxonomy 50 records, spec-first ordering spec-only on all six first commits, version consistency 0.86.0 across all four CI-checked locations, and label-at-creation within 1–2s on all five labelled PRs. *Tests must pass* remains `unverified` by design — there is no application suite. Two constraints were not triggered (no PR added a skill, agent or command) and the Linux leg of *Layer 0 on macOS and Linux* is CI evidence rather than observed here.)
