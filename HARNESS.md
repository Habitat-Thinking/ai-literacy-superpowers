...
Constraints enforced: 31/32
Garbage collection active: 19/19
Drift detected: no (32 active constraints — 22 deterministic + 9 agent + 1 unverified; only "Tests must pass" is unverified, by design (no app suite). Every declared enforcement tool/workflow/script exists on HEAD — no declared-but-missing enforcement found. The count grew from 31 to 32 since the 2026-06-23 audit with the "Sentinel integrity" deterministic constraint added in PR #482: sentinel-integrity-check.sh exists and is wired both at PR time (.github/workflows/harness.yml) and weekly (.github/workflows/gc.yml), rejecting Write/Edit tools on any role: sentinel agent and any out-of-enum role: value. "Convention parity" deterministic constraint (scripts/check-convention-parity.py + convention-parity-check.yml) passes at 32/32 — every active HARNESS.md constraint heading appears verbatim in all three convention files. Affordance↔permission constraints active: gh-cli affordance + committed Bash(gh *) grant in .claude/settings.json; harness-affordance-check.sh passes both --direction=blocking and --direction=advisory. "Layer 0 bash tests run on macOS and Linux" is deterministic — tdad-tests-fast.yml carries a layer0-macos job on macos-latest plus the ubuntu-latest fast-suite. Latest snapshot 2026-07-21 (current, within the monthly 30-day cadence). Template-version marker in sync with plugin.json at 0.66.2 (reconciled via /harness-upgrade on 2026-07-22; the template gained no new adoptable content since 0.64.0 — its own content marker is 0.57.0 — so only the marker moved). Reflection log: 54 active fragments, 0 archived, 0 entries older than 180 days (oldest 2026-04-06; no curation debt); Path 1 (deterministic auto-archive) and Path 2 (agent aged-out review) GC rules declared and operating. Affordance GC rules remain commented-out by design.)
